<?php
class ShapeShifter_New_Entries extends SSE_Widget {
	...
	function __construct() {
		...
		parent::__construct( false, $name = esc_html__( 'ShapeShifter New Posts List', ShapeShifter_Extensions::TEXTDOMAIN ) );//ShapeShifter 新着記事一覧
	...
	function widget( $args, $instance ) {
		...
		$this->post_formats = $post_format_terms = null;

		$entry_count = intval( $instance['entry_count'] );
		$entry_count = ( $entry_count ? $entry_count : 5 );
		...
		if( isset( $category_id[ 0 ] ) ) {
			$widget_new_args['category__in'] = $category_id;
		}
		$entry_count = $category_id = $tax_query = null;
		
		$posts = get_posts( $widget_new_args ); $widget_new_args = null;

		if( ! isset( $posts[ 0 ] ) ) return;

		echo $before_widget; $before_widget = null;
			$title_new = esc_html( strip_tags( $instance['title_new'] ) );
			$title_not_display = ( ! empty( $instance['title_not_display'] ) ? true : false );
			if( ! $title_not_display ) { 
				if( $title_new != '' ) { 
					echo $before_title . '<span class="title-new-entry">' . $title_new . '</span>' . $after_title;
				} 
			} $title_not_display = $before_title = $title_new = $after_title = null; 			
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['title_new'] = $instance['title'] = sanitize_text_field( strip_tags( $new_instance['title_new'] ) );
	...
	function form( $instance ) {
		...
		foreach( $this->post_formats as $post_format => $text ) {
			...
			$display_format['display_format_' . $post_format ] = 'display_format_' . $post_format;
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title_new' ) ); ?>">
		...
		<div id="widget-category-settings-box-<?php echo esc_attr( $this->number ); ?>" class="widget-settings-box widget-vegas-background-images-settings-box">
			...
			<p><strong><?php esc_html_e( 'Specify the Categories to display', ShapeShifter_Extensions::TEXTDOMAIN ); ?></strong></p>
			<?php		
			if( $this->terms ) {
				foreach( $this->terms as $term ) {
					if ( $term->count > 0 ) {
