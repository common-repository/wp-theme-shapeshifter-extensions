...
# Making Directory
#
	if( ! function_exists( 'sse_mkds' ) ) { 
		/**
		 * Make Directory
		 * 
		 * @parma string $directory
		 * 
		 * @return bool
		**/
		function sse_mkds( $directory ) {

			if( current_user_can( 'update_plugins' ) ) return false;

			if( ! is_dir( $directory ) ) {

				if( ! sse_mkds( dirname( $directory ) ) )
					return false;
...
# String
#
	if ( ! function_exists( 'sse_is_json_string' ) ) {
		/**
		 ...
		 * @return bool
		**/
		function sse_is_json_string( $var ) {

			json_decode( $var );
			return shapeshifter_boolval( json_last_error() == JSON_ERROR_NONE );
